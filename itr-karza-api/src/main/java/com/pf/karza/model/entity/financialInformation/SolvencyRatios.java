...
@Entity
@Table(name = "financial_information_ratios_solvency_ratios", schema = DbConstants.ITR_SCHEMA_NAME)
@Getter
@Setter
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class SolvencyRatios extends BaseId {
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonBackReference
    @JoinColumn(nullable = false, name = "financial_information_ratios_id")
    private Ratios ratios;

    @Column(name = "equity_multiplier")
    private String equityMultiplier;

    @Column(name = "debt_capital")
    private String debtCapital;

    @Column(name = "ebitda_coverage")
    private String ebitdaCoverage;

    @Column(name = "debt_equity")
    private String debtEquity;

    @Column(name = "debt_ebitda")
    private String debtEBITDA;
