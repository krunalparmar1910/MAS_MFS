...
@Entity
@Table(name = "filling_data_financial_info_asset_liability_details", schema = DbConstants.ITR_SCHEMA_NAME)
@Getter
@Setter
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class AssetLiabilityDetails extends BaseId {
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    @JoinColumn(nullable = false, name = "financial_info_id", referencedColumnName = "id")
    @JsonBackReference
    private FinancialInfo financialInfo;

    @Column(name = "immovable_assets")
    private String immovableAssets;

    @Column(name = "movable_assets")
